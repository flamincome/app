...
async function createSwap(web3: Web3,  invoice: string, paymentHash:string, tBTCAmount:string, operatorAddress:string) {
    const ln2tbtcContract: Ln2tbtcContract = new web3.eth.Contract([] as AbiItem[], '');
    const tbtcContract: ERC20Contract = new web3.eth.Contract([] as AbiItem[], '');
    const userAddress = (web3.currentProvider as any).selectedAddress;
    await tbtcContract.methods.approve('', tBTCAmount).send({
        from: userAddress
    })
